namespace ReverseTicTacToe.UI
{
    ...
    public partial class FormGameBoard : Form
    {
        ...
        public FormGameBoard(byte i_BoardSize, ReverseTicTacToeGame i_ReverseTicTacToeGame)
        {
            ...
            InitializeComponent();
            updateFormGameBoard(rightLocation, bottomLocation,
                i_ReverseTicTacToeGame.PlayerOne.Name, i_ReverseTicTacToeGame.PlayerTwo.Name);
        }
        ...
        private BoardCell chooseMoveToPlay(BoardGameButton i_ButtonClicked)
        {
            ...
            if (m_ReverseTicTacToeGame.IsComputerPlaying == true && 
                m_ReverseTicTacToeGame.CurrentPlayer == m_ReverseTicTacToeGame.PlayerTwo)
            {
        ...
        private void playCurrentPlayerTurn(BoardGameButton i_ButtonClicked)
        {
            ...
            while(BoardGameButtons[boardCell.Row, boardCell.Col].Enabled == false)
            {
                boardCell = chooseMoveToPlay(i_ButtonClicked);
            }

            BoardGameButtons[boardCell.Row, boardCell.Col].Text = char.ToString(ReverseTicTacToeGame.CurrentPlayer.Sign);
            BoardGameButtons[boardCell.Row, boardCell.Col].Enabled = false;
            ReverseTicTacToeGame.PlayTurn(boardCell);
            gameStatusMessage();
        ...
        private void Button_Click(object sender, EventArgs e)
        {
            ...
            if(m_ReverseTicTacToeGame.CurrentPlayer == m_ReverseTicTacToeGame.PlayerTwo &&
                m_ReverseTicTacToeGame.IsComputerPlaying == true)
            {
                playCurrentPlayerTurn(button);
            }
        }
        ...
        private void gameStatusMessage()
        {
            ...
            switch (ReverseTicTacToeGame.StatusGame)
            {
                case eCurrentStatusOfTheGame.PlayerOneLost:
                    dialogResult = MessageBox.Show(string.Format("The winner is {0}!{1}{2}",
                        ReverseTicTacToeGame.PlayerTwo.Name, Environment.NewLine, AnotherRound), "A Win!", MessageBoxButtons.YesNo);
                    break;

                case eCurrentStatusOfTheGame.PlayerTwoLost:
                    dialogResult = MessageBox.Show(string.Format("The winner is {0}!{1}{2}",
                        ReverseTicTacToeGame.PlayerOne.Name, Environment.NewLine, AnotherRound), "A Win!", MessageBoxButtons.YesNo);
                    break;

                case eCurrentStatusOfTheGame.ItIsADraw:
                    dialogResult = MessageBox.Show(string.Format("Tie!{0}{1}", Environment.NewLine, AnotherRound),
            ...
            {
                MessageBox.Show(string.Format("See you next time!{0}The score is {1} {2}:{3} {4}", Environment.NewLine,
                    ReverseTicTacToeGame.PlayerOne.Name, ReverseTicTacToeGame.PlayerOne.Score,
                    ReverseTicTacToeGame.PlayerTwo.Score, ReverseTicTacToeGame.PlayerTwo.Name),"Bye Bye!", MessageBoxButtons.OK);
                DialogResult = DialogResult.OK;
                this.Close();
            }
        }

        private void updateFormGameBoard(int i_FormWidth, int i_FormHeight,
        ...
        {
            return string.Format("{0} : {1}", ReverseTicTacToeGame.PlayerOne.Score,
                ReverseTicTacToeGame.PlayerTwo.Score);
        }
        ...
        private void setButtonsOnBoardGame(byte i_BoardSize,
            out int io_RightLocation, out int io_BottomLocation)
        {
